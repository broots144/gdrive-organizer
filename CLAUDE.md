...
5. **Write rules, not per-file decisions.** Express the taxonomy as rules (SQL predicate over
   ...
   `gdrive-organizer plan`, then run `validate`. Prefer folder-level moves. Never split an
   ...
   `gdrive-organizer dedupe` (each op names the byte-identical copy that stays), or empty folders
   from `gdrive-organizer empty-dirs`.
...
- `plan.py`: rules file to manifest (collisions, merges, gates, counts only)
- `dedupe.py`: exact duplicates to trash ops under a keep/never-trash policy
- `empty_dirs.py`: empty folders to trash ops, innermost first
- `examples/demo/`: a self-contained demo on a fake local tree (no Google account)
