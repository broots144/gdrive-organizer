"""Turn empty folders in an index into a manifest of trash ops for `gdrive-organizer validate`.
  ...
  gdrive-organizer empty-dirs --db private/index.sqlite --config private/config.json \
...
file (the same one `gdrive-organizer dedupe` uses):
...
PROG = "gdrive-organizer empty-dirs"
